...
## Rust HAL Sketch (esp-hal TWAI)

```rust
use esp_hal::twai::{Twai, TwaiConfiguration, BaudRate};

pub fn init_can(twai: esp_hal::peripherals::TWAI0) -> Twai<'static> {
    let config = TwaiConfiguration::new(
        twai,
        BaudRate::B500K,
        /* tx_pin, rx_pin configured in board setup */
    );
    config.start()
}

pub fn send_frame(can: &mut Twai, id: u16, data: &[u8; 8], len: u8) {
    // esp-hal provides frame types — illustrative API
    let frame = esp_hal::twai::TwaiFrame::new_standard(id, &data[..len as usize]);
    let _ = can.transmit(&frame);
}
```

**Ownership:** One `Twai` instance owns the controller; receive often uses interrupt + channel.
...
```rust
// TWAI register access via PAC — configure bit timing:
...
2. Parse a DBC file for one signal in Rust (`can-dbc` crate).
...
- [socketcan-rs](https://docs.rs/socketcan/) (Linux); embedded: `embedded-can` trait
