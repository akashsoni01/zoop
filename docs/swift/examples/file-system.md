...
**Goal:** LittleFS on flash — hands-on Embedded Swift.
...
```swift
var fs = LittleFS.mount(partition: .data)
try fs.write(path: "/config.json", data: configBytes)
```

### Ownership / ARC Notes

- File handles as structs; close explicitly before sleep.
