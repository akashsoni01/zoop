...
**Goal:** Analog input — hands-on Embedded Swift.

**Prerequisites:** See core lessons and [communication/12-adc.md](../12-adc.md) if applicable.

**Protocol:** [communication/12-adc.md](../12-adc.md)
...
```swift
var adc = ADC1.channel0(attenuation: .db11)
let raw = adc.read()
let millivolts = adc.rawToMillivolts(raw)
```

### Ownership / ARC Notes

- Sample buffer can be stack-allocated `[UInt16]`.
