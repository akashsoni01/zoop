...
**Goal:** Fast SPI/memory — hands-on Embedded Swift.
...
```swift
var dma = DMA.channel0()
dma.start(source: spiRxBuffer, dest: displayBuffer, length: 4096)
while !dma.isComplete { }
```

### Ownership / ARC Notes

- Ensure buffer lifetime exceeds DMA transfer — pin buffer in outer scope.
