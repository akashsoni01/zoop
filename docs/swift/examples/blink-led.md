...
**Goal:** First output — hands-on Embedded Swift.
...
struct BlinkApp {
    static func main() -> Never {
        var led = GPIO.Output<GPIO48>(initial: .low)
        while true {
            led.toggle()
...
- GPIO48 is moved into `Output` — exclusive ownership prevents double-configuring the pin.
