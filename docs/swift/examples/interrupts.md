...
**Goal:** ISR handlers — hands-on Embedded Swift.
...
```swift
func gpioISR() {
    // Keep ISR minimal — set flag only
    ButtonEvent.pending = true
}

// Main loop
while true {
    if ButtonEvent.pending {
        ButtonEvent.pending = false
        handleButton()
    }
}
```
...
- Never allocate or retain in ISR. Use `volatile` or atomic flags.
