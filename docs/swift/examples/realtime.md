# Example: Real-Time Patterns

**Goal:** Deterministic timing — hands-on Embedded Swift.

**Prerequisites:** See core lessons and [communication/23-rtic.md](../23-rtic.md) if applicable.

**Protocol:** [communication/23-rtic.md](../23-rtic.md)
...
```swift
// Fixed-period loop with drift compensation
var nextWake = Timer.now()
while true {
    sampleAndControl()
    nextWake &+= periodUs
    Timer.sleepUntil(nextWake)
}
```

### Ownership / ARC Notes

- Avoid heap and ARC in control loop — preallocate all buffers at init.
