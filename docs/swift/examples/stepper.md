...
**Goal:** Step/dir driver — hands-on Embedded Swift.
...
```swift
struct Stepper {
    var step, dir: GPIO.Output<...>
    mutating func move(steps: Int, clockwise: Bool) {
        dir.set(clockwise)
        for _ in 0..<steps { step.pulse() }
    }
}
```
...
- Step timing via busy-wait or timer — not Task.sleep in bare-metal.
